...
import {MiniSales} from '../sales/components';

const FilteredBatches = () => {
  const routeData = useRoute<any>().params?.data;
  const isSales = useRoute<any>().params?.fromSales;
  console.log('🚀 ~ FilteredBatches ~ isSales:', isSales);
  ...
  const handleDeleteBatch = async (id?: any) => {
    ...
    apiCall(url, 'DELETE')
      .then(async res => {
        showPopupWithOk(
          'Success',
          `${isSales ? 'Sales Record' : 'Batch'} deleted successfully`,
          () => {},
        );
        setPage(1);
        timer = setTimeout(async () => {
